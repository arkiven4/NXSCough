...
class OPERAGT_MAE(nn.Module):
    def __init__(self, input_size, img_size=(256, 64), patch_size=4, mask_ratio=0.7, 
        ...
        if use_custom_patch:
            ...
            self.patch_embed = modules.PatchEmbed_new(img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim, stride=stride)
        else:
    ...
    def initialize_weights(self):
        ...
        pos_embed = modules.get_2d_sincos_pos_embed_flexible(self.pos_embed.shape[-1], self.patch_embed.patch_hw, cls_token=True)  
        self.pos_embed.data.copy_(torch.from_numpy(pos_embed).float().unsqueeze(0))

        decoder_pos_embed = modules.get_2d_sincos_pos_embed_flexible(self.decoder_pos_embed.shape[-1], self.patch_embed.patch_hw, cls_token=True)

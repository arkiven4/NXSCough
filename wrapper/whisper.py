...
class WhisperEncoderLayer(nn.Module):
    def __init__(self, config):
        ...
        if self.config.finetune_method == "lora" or self.config.finetune_method == "combined":
            ...
            # --- LoRA for attention projections ---
            self.self_attn.q_proj = lora.Linear(
                self.embed_dim, self.embed_dim, r=config.lora_rank, bias=False
            )
            self.self_attn.k_proj = lora.Linear(
                self.embed_dim, self.embed_dim, r=config.lora_rank, bias=False
            )
            self.self_attn.v_proj = lora.Linear(
                self.embed_dim, self.embed_dim, r=config.lora_rank, bias=False
            )
            self.self_attn.out_proj = lora.Linear(
                self.embed_dim, self.embed_dim, r=config.lora_rank, bias=False
            )
...
class WhisperWrapper(nn.Module):
    def __init__(
        ...
        self.feature_extractor = AutoFeatureExtractor.from_pretrained("openai/whisper-medium")
        ...
        self.model_config.embedding_prompt_dim   = model_config.embedding_prompt_dim
        self.model_config.lora_rank              = model_config.lora_rank
        self.hidden_size_ssl = self.backbone_model.config.hidden_size
        ...
        #     nn.Conv1d(self.model_config.hidden_size, self.config.pooling_hidden, 1, padding=0),
        #     nn.ReLU(),
        #     nn.Dropout(p=0.1),
        #     nn.Conv1d(self.config.pooling_hidden, self.config.pooling_hidden, 1, padding=0),
        #     nn.ReLU(),
        #     nn.Dropout(p=0.1),
        #     nn.Conv1d(self.config.pooling_hidden, self.config.pooling_hidden, 1, padding=0)
        # )
        # if self.config.use_conv_output:
    ...
    def forward(self, x, attention_mask=None):
        ...
        if attention_mask is not None:
            wav_lens = torch.sum(attention_mask, dim=1)
            max_audio_len = wav_lens.max().detach().cpu()
            # Append to list for feature_extractor to work
            new_x = list()
            for idx in range(len(wav_lens)):
        ...
        max_len = int(math.floor((features.shape[-1] / 2) + 0.5))
        ...
        features = self.model_pooling(features, attention_mask)
        
        # 8. Output predictions
        # B x D
        predicted = self.model_reg(features)
        return predicted

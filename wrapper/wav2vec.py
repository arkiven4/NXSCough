...
class Wav2Vec2EncoderLayer(nn.Module):
    def __init__(
        ...
        if self.config.finetune_method == "lora" or self.config.finetune_method == "combined":
            ...
            self.attention.q_proj = lora.Linear(config.hidden_size, config.hidden_size, r=config.lora_rank)
            self.attention.k_proj = lora.Linear(config.hidden_size, config.hidden_size, r=config.lora_rank)
            self.attention.v_proj = lora.Linear(config.hidden_size, config.hidden_size, r=config.lora_rank)
            self.attention.out_proj = lora.Linear(config.hidden_size, config.hidden_size, r=config.lora_rank)

...
for fold, (train_idx, val_idx) in enumerate(splitter):
    ...
    train_loader = DataLoader(train_dataset, num_workers=28, batch_sampler=sampler,
    ...
    runner_lightning = lightning_wrapper.CoughClassificationRunner(
        pool_model, hps=hps, custom_logger=logger, class_weights=class_weights_tensor)
    trainer = L.Trainer(
        max_epochs=1000,
